...
export const envSchema = z
	...
	.superRefine((data, ctx) => {
		...
		const sign = data.ALIYUN_SMS_SIGN_NAME?.trim();
		const template = data.ALIYUN_SMS_TEMPLATE_CODE?.trim();
		if (!sign) {
			ctx.addIssue({
				code: "custom",
				message:
					"When SMS_ENABLED is true, ALIYUN_SMS_SIGN_NAME must be a non-empty string.",
		...
		if (!template) {
			ctx.addIssue({
				code: "custom",
				message:
					"When SMS_ENABLED is true, ALIYUN_SMS_TEMPLATE_CODE must be a non-empty string.",
